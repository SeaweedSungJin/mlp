...
def _load_embedding_projector(
    ...
    *,
    ...
    w4 = state_dict.get("net.4.weight", None)
    if w0 is None or w4 is None:
        raise KeyError("Projector checkpoint must contain 'net.0.weight' and 'net.4.weight'.")

    hidden_dim, input_dim = tuple(w0.shape)
    output_dim, hidden_dim_2 = tuple(w4.shape)
    if hidden_dim_2 != hidden_dim:
        raise ValueError(f"Projector hidden_dim mismatch: net.0.weight={w0.shape}, net.4.weight={w4.shape}")

    proj = EmbeddingProjector(input_dim=input_dim, output_dim=output_dim, hidden_dim=hidden_dim, dropout=0.1)
