...
class BgeHFTextReranker:
    """Text-only reranker using HuggingFace BGE reranker models."""
    ...
    def rerank(
        ...
        *,
        ...
        When return_indices=True, also returns the sorted indices w.r.t. the input sections.
        """
        if not sections:
        ...
        for start in range(0, len(sections), self.batch_size):
            ...
            pairs = [[question, sec] for sec in batch_sections]
        ...
        scores_sorted = [float(scores[int(i)]) for i in sorted_idx]
    ...
    def rerank_entry_sections(
        ...
        """Compatibility wrapper mirroring legacy BGE reranker API."""
        ranked, _ = self.rerank(question, sections[:top_k])
        if not ranked:
            return -1, 0
