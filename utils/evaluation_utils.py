...
_QUESTION_TYPES = ['templated', 'automatic', 'multi_answer', '2_hop', 'infoseek']
_DIGIT_MAP = {
    'none': '0',
    'zero': '0',
    'one': '1',
...
def initialize_bem_scoring_function(vocab_path=_VOCAB_PATH,
  ...
  # def pad(a, length=512):
  #   pad_len = max(0, length - a.shape[-1])
  #   if pad_len == 0:
  #       return a[:length]
  #   return np.append(a, np.zeros(pad_len, np.int32))
  def pad(a, length=512):
    # a: (1, L) 형태라고 가정
    a = np.asarray(a)
    if a.ndim == 2:
    ...
    if L >= length:
        ...
        pad_len = length - L
        seq = np.append(seq, np.zeros(pad_len, np.int32))
  ...
  def bertify_examples(examples):
    ...
    for example in examples:
      example_inputs = preprocess_example(example)
      input_ids.append(pad(example_inputs['input_ids']))       # (512,)
      segment_ids.append(pad(example_inputs['segment_ids']))   # (512,)

    return {
        'input_ids': np.stack(input_ids),      # (batch, 512)
        'segment_ids': np.stack(segment_ids),  # (batch, 512)
    }
...
def encyclopedic_vqa_evaluation_function(example,
    ...
    if not str(example.get("reference", "")).strip():
        return None   # 상위 loop에서 continue 처리
