...
def inference_once(args):
    ...
    images_tensor = process_images(images, args.image_processor, args.model.config).to(args.device, dtype=torch.float16)
    ...
    stop_str = conv.sep if conv.sep_style != SeparatorStyle.TWO else conv.sep2
    # Input
    input_ids = (tokenizer_image_token(prompt, args.tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt").unsqueeze(0).cuda())
    stopping_criteria = KeywordsStoppingCriteria([stop_str], args.tokenizer, input_ids)
    # Inference
    with torch.inference_mode():
    ...
    if n_diff_input_output > 0:
        print(f"[Warning] {n_diff_input_output} output_ids are not the same as the input_ids")
    outputs = args.tokenizer.batch_decode(output_ids[:, input_token_len:], skip_special_tokens=True)[0]
    outputs = outputs.strip()
    if outputs.endswith(stop_str):
        outputs = outputs[: -len(stop_str)]
    # print(f"🚀 {model_name}: {outputs.strip()}\n")
