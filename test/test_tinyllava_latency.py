...
from transformers import AutoProcessor, AutoTokenizer, AutoModelForCausalLM,LlavaForConditionalGeneration
...
iNat_image_path = "/data/dataset/inaturalist"
...
def run_test(
    ...
    **kwargs,
    ...
    if kwargs["resume_from"] is not None:
        ...
        retriever_cfg_path = Path(kwargs.get("retriever_cfg", "config/retriever_config.yaml"))
    ...
    if need_generation:
        ...
        # ---- LLaVA 로드 (cuda:0) ----
        print(f"[LLaVA] tinyllava/TinyLLaVA-Gemma-SigLIP-2.4B on {llava_device}")
        llava_model_id = "tinyllava/TinyLLaVA-Gemma-SigLIP-2.4B"
        # print(f"[LLaVA] tinyllava/TinyLLaVA-Phi-2-SigLIP-3.1B on {llava_device}")
        # llava_model_id = "tinyllava/TinyLLaVA-Phi-2-SigLIP-3.1B"
        ...
        llava_model.cuda()
        config = llava_model.config
        tokenizer = AutoTokenizer.from_pretrained(llava_model_id, use_fast=False, model_max_length = config.tokenizer_model_max_length,padding_side = config.tokenizer_padding_side)
    ...
    def _generate_answer_with_image(
        ...
        *,
        ...
        """Generate answer using LLaVA (image + text + optional context)."""
        # max_tokens = int(max_new_tokens or 64)
        answer = None
        try:
            raw_image = Image.open(image_path).convert("RGB")

            # '<image>' 토큰은 금지. content에 image 타입으로만 포함.
            if context_text.strip():
                user_text = (
                    f"Context : {context_text}\n"
                    f"{question} Use context.\n"
                    # "Answer concisely using the context and the image."
                    # f"Short answer is:"
                )
            else:
            ...
            with torch.inference_mode():
                answer, genertaion_time = llava_model.chat(
                    prompt=user_text, image=image_path, tokenizer=tokenizer)

            # answer = llava_processor.decode(output[0][2:], skip_special_tokens=True).strip()

            #=== Post-process: only keep content after "ASSISTANT:" ===
    ...
    # --- latency / memory / FLOPs 설정 ---
    profile_inference: bool = bool(kwargs.get("profile_inference", False))
    warmup: int = int(kwargs.get("profile_warmup", 10))  # 처음 10개 예열

    latencies_ms: List[float] = []
    gpu_peaks_mb: List[float] = []
    flops_list: List[float] = []  # per-query FLOPs
        ...
        if need_generation:
            if profile_inference:
                ...
                if torch.cuda.is_available():
                    torch.cuda.reset_peak_memory_stats(llava_device)
                    torch.cuda.synchronize(llava_device)

                start = time.perf_counter()
                # FLOPs + latency를 같이 측정
                # with profile(
                ...
                # warmup 개수(예: 처음 10개)는 통계에서 제외
                if processed_idx >= warmup:
                    latencies_ms.append(elapsed_ms)
                    # flops_list.append(total_flops)
                    if torch.cuda.is_available():
                        peak_mb = torch.cuda.max_memory_allocated(llava_device) / (1024 ** 2)
                        gpu_peaks_mb.append(peak_mb)
            else:
                # 기존 동작 (측정 없이 바로 호출)
                answer = _generate_answer_with_image(
                    llava_model,
    ...
    if profile_inference and latencies_ms:
        print("===== Inference Profiling (TinyLLaVA) =====")
        print(f"warmup samples skipped: {warmup}")
        print(f"measured samples: {len(latencies_ms)}")

        print(
            f"Latency(ms): mean={stats.mean(latencies_ms):.2f}, "
            f"stdev={stats.pstdev(latencies_ms):.2f}, "
            f"min={min(latencies_ms):.2f}, max={max(latencies_ms):.2f}"
        )
        if gpu_peaks_mb:
            print(
                f"GPU peak mem(MB): mean={stats.mean(gpu_peaks_mb):.1f}, "
                f"min={min(gpu_peaks_mb):.1f}, max={max(gpu_peaks_mb):.1f}"
            )

        # if flops_list:
        #     gflops = [f / 1e9 for f in flops_list]
        #     print(
        #         f"FLOPs (GFLOPs per query): "
        #         f"mean={stats.mean(gflops):.2f}, "
        #         f"stdev={stats.pstdev(gflops):.2f}, "
        #         f"min={min(gflops):.2f}, max={max(gflops):.2f}"
        #     )
