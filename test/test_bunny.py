...
# === Keras backend 호환 패치 (Bunny가 keras.backend.is_tensor를 쓸 때 대비) ===
try:
    import keras
    import keras.backend as K
    print("[PATCH] keras version:", getattr(keras, "__version__", "unknown"))

    if not hasattr(K, "is_tensor"):
        print("[PATCH] keras.backend.is_tensor 가 없어서 fallback을 정의합니다.")
        ...
        def _is_tensor(x):
            # TensorFlow 있으면 그거 쓰고, 아니면 그냥 False 반환
            if tf is not None:
                try:
        ...
        K.is_tensor = _is_tensor

    else:
        print("[PATCH] keras.backend.is_tensor 이미 존재합니다.")
except Exception as e:
    print("[PATCH] keras backend patch 실패:", e)
# =======================================================================
    ...
    **kwargs,
    ...
    if evaluate_answers:
        try:
            ...
            print(f"[WARN] VQA evaluation disabled (failed to load evaluation_utils / TF deps): {e}")
            evaluate_example_fn = None
    else:
        evaluate_example_fn = None
    ...
    if need_generation:
        # ---- 장치 고정 ----
         # Bunny 설정
        bunny_model_name = kwargs.get("model_path", "BAAI/Bunny-v1_1-Llama-3-8B-V")
        bunny_device_str = kwargs.get("llm_device", "cuda:0")
        bunny_device = torch.device(bunny_device_str if torch.cuda.is_available() else "cpu")

        # v1.1 Llama-3-8B-V, v1.1-4B들은 offset_bos = 1 (README 참고)
        bunny_offset_bos = 1

        print(f"[Bunny] Loading {bunny_model_name} on {bunny_device}")
        bunny_model = AutoModelForCausalLM.from_pretrained(
            bunny_model_name,
            torch_dtype=torch.float16 if bunny_device.type == "cuda" else torch.float32,
            device_map="auto" if bunny_device.type == "cuda" else None,
        ...
        *,
        ...
        try:
            ...
            if not hasattr(K, "is_tensor"):
                ...
                K.is_tensor = lambda x: tf_is_tensor(x)
        except Exception:
            pass
        
        """Generate answer using Bunny (image + optional context + question)."""
        ...
        try:
            ...
            if context_text.strip():
                prompt = (
                    ...
                    "Use the external textual context as the primary factual source, and the image as auxiliary visual evidence.\n\n"
                    f"Context:\n{context_text.strip()}\n\n"
                    f"Question:\n{question.strip()}\n"
            ...
            # Bunny는 <image> 토큰을 텍스트 안에 넣고, -200으로 placeholder를 채움
            text = (
            ...
            input_ids = torch.tensor(
                text_chunks[0] + [-200] + text_chunks[1][offset_bos:],
                dtype=torch.long,
            ).unsqueeze(0).to(bunny_device)

            image_tensor = bunny_model.process_images([image], bunny_model.config).to(
            ...
            print(f"[Bunny] generation failed: {e}")
            answer = None
    ...
    for processed_idx, dataset_idx in tqdm(
        ...
        try:
            ...
            print(f"[WARN] skipping sample {dataset_idx}: {e}")
            continue
        # -------------------------
        ...
        if use_rag:
            ...
            if reranker_type == "qformer" and kwargs["perform_qformer_reranker"] and sections:
