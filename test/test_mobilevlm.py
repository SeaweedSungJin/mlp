...
def run_test(
    ...
    **kwargs,
    ...
    # === Answer generator ===
    if need_generation:
        mobilevlm_device_str = 'cuda:0'
        mobilevlm_device = torch.device(mobilevlm_device_str if torch.cuda.is_available() else "cpu")

        from mobilevlm.mobilevlm.model.mobilevlm import load_pretrained_model
        mobilevlm_model_path = kwargs.get("model_path", "mtgv/MobileVLM-3B")
        mobilevlm_tokenizer, mobilevlm_model, mobilevlm_image_processor, _ = load_pretrained_model(
            mobilevlm_model_path, device_map={"": mobilevlm_device_str}
        )

    # === Helper functions ===
    def _generate_answer_with_image(
        mobilevlm_model,
        mobilevlm_image_processor,
        mobilevl_tokenizer,
        device: torch.device,
        question: str,
        image_path: str,
        ...
        try:
            ...
            if context_text.strip():
                user_text = (
                    ...
                    "Use the image and the context below to answer the question.\n"
                    "Follow the context strictly when factual details are involved.\n"
                    f"\nContext:\n{context_text.strip()}\n"
            ...
            args = type('Args', (), {
                "model": mobilevlm_model,
                "tokenizer" : mobilevl_tokenizer,
                "image_processor":mobilevlm_image_processor,
            ...
            answer = inference_once(args)
            
            torch.cuda.empty_cache()
        except Exception as e:
            print(f"[mobilevlm] generation failed: {e}")
            answer = None
